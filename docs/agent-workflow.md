...
1. If the task references a feature, plan, or prior decision, check `specs/` filenames and read only the matching spec. `specs/_template.md` is the template for new specs.
...
Project structure is in [Architecture](architecture.md). Scope rules are in [Change Policy](change-policy.md).
