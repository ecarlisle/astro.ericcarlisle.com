...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		name: "astro_check",
		label: "Astro Check",
		description: "Run `pnpm typecheck` (TS + Astro diagnostics). Use after schema, component, or utility changes.",
	...
	pi.registerTool({
		...
		description: "Validate blog frontmatter against src/content.config.ts. Use after adding or editing posts.",
		parameters: Type.Object({}),
		async execute() {
			return {
				content: [
	...
	pi.registerTool({
		...
		async execute() {
			return {
				content: [{ type: "text", text: "Run `pnpm build`, then `pnpm preview` to serve the dist/ output." }],
				details: {},
			};
		},
	});
	...
	pi.registerTool({
		...
		description: "Run full Lighthouse audit. Use for performance, accessibility, SEO, or best-practices changes.",
		parameters: Type.Object({}),
		async execute() {
			return {
				content: [
	...
	pi.registerTool({
		...
		async execute() {
			return {
				content: [{ type: "text", text: "Run `pnpm structured-data:report` to validate JSON-LD output." }],
				details: {},
			};
		},
	});
